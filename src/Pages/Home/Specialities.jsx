...
function Specialities() {
  ...
  const reverseTextVariants = useMemo(
    () => ({
      hidden: { opacity: 0, x: 50 },
      visible: {
        opacity: 1,
  ...
  const reverseAnimationVariants = useMemo(
    () => ({
      hidden: { opacity: 0, x: -50 },
      visible: {
        opacity: 1,
  ...
  // Memoized data
  const specialitiesData = useMemo(
    () => [
  ...
  const physiotherapyData = useMemo(
    () => [
      ...
      <div className="container-responsive section-padding">
        ...
        >
          ...
          <motion.div variants={textVariants} className="space-y-8">
            <motion.h2 className="text-responsive-h2 font-bold mb-8 sm:mb-12 p-4 sm:p-8 lg:p-12 text-white bg-clip-text">
              અમારી<span className="text-blue-300"> વિશેષતાઓ</span>
            </motion.h2>

            <motion.p className="text-responsive-body text-gray-300 leading-relaxed mb-6 sm:mb-8">
              અમારી ક્લિનિકમાં આપને મળશે વિશ્વસનીય અને ગુણવત્તાયુક્ત
              ફિઝિયોથેરાપી સેવાઓ
            </motion.p>

            <motion.div className="space-y-6">
              {specialitiesData.map((item, index) => (
                <motion.div key={index} className="flex items-center space-x-4">
          ...
          >
            <div className="relative w-full max-w-lg h-64 sm:h-80 lg:h-96">
              ...
              <div className="absolute -bottom-4 -left-4 w-6 h-6 bg-pink-400 rounded-full  delay-1000 opacity-60"></div>
              <div className="absolute top-1/2 -left-8 w-4 h-4 bg-green-400 rounded-full  delay-500 opacity-60"></div>
              <div className="absolute top-1/2 -right-8 w-4 h-4 bg-cyan-400 rounded-full  delay-1500 opacity-60"></div>
            </div>
          </motion.div>
        </motion.div>
      </div>
        ...
        >
          ...
          >
            <div className="relative w-full max-w-lg h-64 sm:h-80 lg:h-96">
              ...
              <div className="absolute -bottom-4 -left-4 w-6 h-6 bg-red-400 rounded-full delay-1000 opacity-60"></div>
              <div className="absolute top-1/2 -left-8 w-4 h-4 bg-yellow-400 rounded-full delay-500 opacity-60"></div>
              <div className="absolute top-1/2 -right-8 w-4 h-4 bg-green-400 rounded-full delay-1500 opacity-60"></div>
            </div>
          </motion.div>

          {/* Right Side - Text Content */}
          <motion.div variants={reverseTextVariants} className="space-y-8">
            <motion.h2 className="text-responsive-h2 font-bold p-4 sm:p-8 lg:p-12 mb-6 sm:mb-8 bg-blue-200 bg-clip-text text-transparent">
              {physiotherapyData[0]}
            </motion.h2>

            <motion.h3 className="text-xl sm:text-2xl lg:text-3xl font-semibold mb-4 sm:mb-6 text-white">
              {physiotherapyData[1]}
            </motion.h3>
